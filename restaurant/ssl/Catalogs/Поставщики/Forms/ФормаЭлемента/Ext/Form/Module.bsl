...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыПоля.ПоказыватьКоманднуюПанель = Ложь;
	 ...
	 // Добавляем еще одну картинку
    ПараметрыПоля2 = РаботаСФайлами.ПолеФайла();    
	ПараметрыПоля2.ПутьКДанным = "Объект.Подпись"; // здесь мы используем другое свойство объекта
	ПараметрыПоля2.ПутьКДаннымИзображения = "АдресПодписи"; // и другой адрес для изображения
	ПараметрыПоля2.Размещение = "Подпись";  // размещаем в другом месте
    ...
    ДобавляемыеЭлементы.Добавить(ПараметрыПоля2); // добавляем новое поле в массив
